...
export function parseKPIDefinitions() {
  ...
  kpiData.forEach((item) => {
    ...
    if (fieldName.startsWith('kpi_') && fieldName.endsWith('_val')) {
      ...
      // This might be continuation data for the current KPI
      // Merge any non-empty fields
      const current = kpiMap[currentKPIKey]
      
      // Check if Source has meaningful data (not empty, not just numbers/calculations)
      const source = item['Source']?.trim() || ''
      if (source && source.length > 20 && !source.match(/^\d+$/) && !source.includes('(X-min)')) {
      ...
      // Check KPI Name for ranking info
      const kpiName = item['KPI Name']?.trim() || ''
      if (kpiName && (kpiName.includes('High to') || kpiName.includes('Low to'))) {
        if (!current.ranking) {
      ...
      if (ranking && (ranking.includes('High to') || ranking.includes('Low to'))) {
        if (!current.ranking) {
      ...
      if (calc && calc !== '(X-min)/(max-min)' && calc !== '(max-X)/(max-min)') {
      ...
      if (indexCalc && (indexCalc.includes('(X-min)') || indexCalc.includes('(max-X)'))) {
  ...
  Object.entries(kpiMap).forEach(([key, kpi]) => {
    ...
    let calculation = (kpi.calculation || '').replace(/^["']|["']$/g, '').trim()
    ...
    if (!ranking || (!ranking.includes('High') && !ranking.includes('Low'))) {
      ...
      const rankingMatch = metricNote.match(/(High to low|Low to high|High to Low|Low to High)/i)
      if (rankingMatch) {
    ...
    if (key === 'diversityIndex' && !ranking.includes('High') && !ranking.includes('Low')) {
    ...
    const format = (val) => {
      if (metricNote.includes('0 to 100%') || metricNote.includes('varies from 0 to 100%') || 
          kpi.kpiName.includes('Footprint') || kpi.kpiName.includes('Equity') || 
          kpi.kpiName.includes('Housing') || kpi.kpiName.includes('Access') || 
          kpi.kpiName.includes('Anchor') || kpi.kpiName.includes('Learning')) {
        return `${val.toFixed(1)}%`
      }
      if (metricNote.includes('0 to 1') || metricNote.includes('varies from 0 to 1') || 
...
export function getFieldNameFromKey(key) {
  const entry = Object.entries(fieldNameToKey).find(([, value]) => value === key)
